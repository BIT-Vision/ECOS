...
class RandomAffine(object):
    ...
    def __call__(self, voxel, image, ret=None):
        """
        ...
        """
        if ret is None:
            ret = self.get_params(self.degrees, self.translate, self.scale, self.shear, voxel.shape[-2:])
        voxel = TF.affine(voxel, *ret, fillcolor=self.fillcolor)
        image = TF.affine(image, *ret, fillcolor=self.fillcolor)
        return voxel, image, ret
...
class RandomResizedCrop(object):
    ...
    def __call__(self, voxel, image):
        ...
        """
        i, j, h, w = self.get_params(voxel, self.scale, self.ratio)
        # print(i, j, h, w)
        voxel = TF.resized_crop(voxel, i, j, h, w, self.size)
        # image = TF.resized_crop(image, i, j, h, w, self.size, interpolation=InterpolationMode.NEAREST)
...
class Resize(torch.nn.Module):
    ...
    def forward(self, voxel, image):
        ...
        """
        voxel = TF.resize(voxel, self.size)
        # image = TF.resize(image, self.size,interpolation=InterpolationMode.NEAREST)
    ...
    def forward_one(self, input):
        ...
        Returns:
            PIL Image or Tensor: Rescaled image.
        """
        input = TF.resize(input, self.size)
        return input
